...
class connector():
    ...
    def __open(self):
        ...
        if 'current' in self._request:
            ...
            self.httpHeader['Content-Disposition'] = disp + '; filename='\
    ...
    def __rename(self):
        ...
        if not curDir or not curName:
            ...
            self._response['error'] = 'File or folder with the same name' +\
                'already exists'
        else:
    ...
    def __path2url(self, path):
        ...
        url = self.__checkUtf8(self._options['URL'] + curDir[length:])\
            .replace(os.sep, '/')
